...
async def connect_to_wss(protocol_proxy, user_id, device_id):
    ...
    while True:
        try:
            ...
            async with proxy_connect(
                ...
                logger.success(f"UID: {truncate_userid(user_id)} | {node_type} | 成功连接到WS | IP: {truncate_proxy(protocol_proxy)} uri: {uri} | 头部信息: {custom_headers} | 设备ID: {device_id} | 代理: {truncate_proxy(protocol_proxy)} | 剩余代理数量: {total_proxies}")

                async def send_ping():
                    while True:
                        logger.debug(f"UID: {truncate_userid(user_id)} | {node_type} | 正在发送PING！{truncate_proxy(protocol_proxy)}")
                        if has_received_action:
                            ...
                            logger.debug(f"UID: {truncate_userid(user_id)} | {node_type} | 发送PING消息 | IP: {truncate_proxy(protocol_proxy)} 数据: {send_message}")
                            await asyncio.sleep(DELAY_INTERVAL)
                            await websocket.send(send_message)
                            logger.info(f"UID: {truncate_userid(user_id)} | {node_type} | 已发送PING | 数据: {send_message}")
                        else:

                            logger.warning(f"UID: {truncate_userid(user_id)} | {node_type} | No Receive Action | IP: {truncate_proxy(protocol_proxy)}")

                        # rand_sleep = 90
                        rand_sleep = random.uniform(80, 100)
                        logger.info(f"UID: {truncate_userid(user_id)} | {node_type} | 下次PING在 {rand_sleep:.2f} 秒后，！{truncate_proxy(protocol_proxy)}")
                ...
                try:
                    while True:
                        ...
                        logger.info(f"UID: {truncate_userid(user_id)} | {node_type} | 接收到消息 | IP: {truncate_proxy(protocol_proxy)} 数据: {message}")
                        ...
                        if message.get("action") == "AUTH":
                            ...
                            logger.debug(f"UID: {truncate_userid(user_id)} | {node_type} | 发送AUTH | 代理: {truncate_proxy(protocol_proxy)} 数据: {auth_response}")
                            await asyncio.sleep(DELAY_INTERVAL)
                            await websocket.send(json.dumps(auth_response))
                            logger.success(f"UID: {truncate_userid(user_id)} | {node_type} | 已发送AUTH | 代理: {truncate_proxy(protocol_proxy)} 数据: {auth_response}")
                            ...
                            async with aiohttp.ClientSession() as session:
                                async with session.get(request_data["url"], headers=headers) as api_response:
                                    ...
                                    logger.info(f"UID: {truncate_userid(user_id)} | {node_type} | 打开PING访问 | IP: {truncate_proxy(protocol_proxy)} 数据: {http_response}")
                                    await asyncio.sleep(DELAY_INTERVAL)
                                    await websocket.send(json.dumps(http_response))
                                    logger.success(f"UID: {truncate_userid(user_id)} | {node_type} | 已发送PING访问 | IP: {truncate_proxy(protocol_proxy)} 数据: {http_response}")

                        elif message.get("action") == "PONG":
                            pong_response = {"id": message["id"], "origin_action": "PONG"}
                            logger.debug(f"UID: {truncate_userid(user_id)} | {node_type} | 发送PONG | 代理: {truncate_proxy(protocol_proxy)} 数据: {pong_response}")
                            await asyncio.sleep(DELAY_INTERVAL)
                            await websocket.send(json.dumps(pong_response))
                            logger.success(f"UID: {truncate_userid(user_id)} | {node_type} | 已发送PONG | 代理: {truncate_proxy(protocol_proxy)} 数据: {pong_response}")
...
async def main():
    ...
    retry_count = {}
    # device_ids = {user_id: str(uuid.uuid4()) for user_id in user_ids}  # 每个用户ID分配一个设备ID
    ...
    for user_id, proxies in proxy_allocation.items():
        for proxy in proxies:
            ...
            await asyncio.sleep(DELAY_INTERVAL)
            task = asyncio.create_task(connect_to_wss(proxy, user_id, device_id))
        ...
        for task in done:
            try:
                ...
                if result in ["skip", None]:
                    ...
                    new_device_id = str(uuid.uuid3(uuid.NAMESPACE_DNS, failed_proxy))
                    new_task = asyncio.create_task(connect_to_wss(failed_proxy, user_id, new_device_id))
                    tasks[new_task] = (failed_proxy, user_id)

                    # temp_proxy = failed_proxy
                    # available_proxies = list(set(all_proxies) - set(proxy_allocation[user_id]))
                    # if available_proxies:
                    #     # logger.warning(f"UID: {truncate_userid(user_id)} | {node_type} | 11111111111111")
                ...
                #         retry_count[(new_proxy, user_id)] = retry_count[(failed_proxy, user_id)]
                #         await asyncio.sleep(DELAY_INTERVAL)
                #         # new_task = asyncio.creadailte_task(connect_to_wss(new_proxy, user_id))
                #         # tasks[new_task] = (new_proxy, user_id)
                ...
                tasks.pop(task)
                # logger.warning(f"UID: {truncate_userid(user_id)} | {node_type} | 3333333333333")
                # new_device_id = str(uuid.uuid3(uuid.NAMESPACE_DNS, temp_proxy))
                # new_task = asyncio.create_task(connect_to_wss(temp_proxy, user_id, new_device_id))
                # tasks[new_task] = (temp_proxy, user_id)

        # active_proxies = [proxy for _, proxy in tasks.values()]
        # for user_id, proxies in proxy_allocation.items():
        #     logger.warning(f"UID: {truncate_userid(user_id)} | {node_type} | 22222222222222")
